...
export interface S1202BuildResult {
  tenantId: string;
  payrollRunId: string;
  employeeId: string;
  xml: string;
...
@Injectable()
export class S1202Builder {
  readonly eventKind = 'S-1202' as const;
  ...
  async build(
    ...
    payrollRunId: string,
    employeeId?: string,
  ): Promise<S1202BuildResult[]> {
    const run = await this.loadRun(tenantId, payrollRunId);
    if (run.status !== 'GENERATED') {
      throw new UnprocessableEntityException(
        'S-1202 emission requires payroll_run.status=GENERATED',
      );
    }
    ...
    const rows = await this.databaseService.query<PayrollItemRow>(
      ...
      JOIN hr.employment_link link
       ...
       AND link.contract_type IN ('statutory', 'commissioned')
      LEFT JOIN LATERAL (
        SELECT cnpj
        FROM hr.company
    ...
    if (rows.length === 0) {
      throw new NotFoundException(
        'Payroll run has no RPPS worker remuneration items',
      );
    }
  ...
  private buildWorker(group: WorkerGroup): S1202BuildResult {
...
<eSocial xmlns="http://www.esocial.gov.br/schema/evt/evtRmnRPPS/v_S_01_03_00">
  <evtRmnRPPS Id="${reference}">
    ...
    <dmDev>
      ...
      <codCateg>${codCateg(group.contractType)}</codCateg>
      <infoPerApur>
        <ideEstab>
          <tpInsc>1</tpInsc>
    ...
    return {
      ...
      payload: {
        ...
        employeeId: group.employeeId,
        ideDmDev,
        codCateg: codCateg(group.contractType),
        rubricCount: group.rubrics.length,
        totalsByRubric: totalsByRubric(group.rubrics),
...
function rubricXml(rubric: S1202Rubric): string {
...
function totalsByKind(rubrics: S1202Rubric[]): Record<string, string> {
...
function totalsByRubric(rubrics: S1202Rubric[]): Record<string, string> {
  return rubrics.reduce<Record<string, string>>((totals, rubric) => {
    totals[rubric.code] = moneyFromCents(
      cents(totals[rubric.code] ?? '0.00') + cents(rubric.amount),
    );
    return totals;
  }, {});
}
...
function codCateg(contractType: string): string {
  return contractType === 'commissioned' ? '302' : '301';
}
